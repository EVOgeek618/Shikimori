...
for c in range(1,151):
  try:
    link=urllib.request.urlopen('https://shikimori.one/animes/order-by/popularity/page/'+str(c))
    lines = []
    ...
    for f in range(20):
        ...
        try:
            ...
            listofanime.setdefault(ng2[ng2.find("/animes/",fg1)+len("/animes/"):fg2],summ)
        except (ValueError):
            continue
        print(wer,ng2[ng2.find("/animes/",fg1)+len("/animes/"):fg2],"|",summ)
  except urllib.error.HTTPError:
      for i in range(1, 61):
          print("\r" + str(i), end='')
          time.sleep(1)
...
n=0
with open('List of popular anime 2020_1_1.txt','w') as comp:
    for i in list_d[::-1]:
        n=n+1
        comp.write(str(n)+"-"+str(i[0])+". "+str(i[1])+"\n")
